...
class ControllerPaymentAdyenIdeal extends Controller {
    ...
    public function index() {
        ...
        $this->data['button_confirm'] = $this->language->get('text_button_ideal');
        $this->data['text_ideal_msg'] = $this->language->get('text_ideal_msg');
        $this->data['testmode'] = $this->config->get('adyen_ideal_test');
        ...
        $this->data['shipBeforeDate'] = $this->config->get('adyen_ideal_days_till_delivery');
        $this->data['session_valid_till'] = 1;
        $this->data['session_valid_till'] = $this->config->get('adyen_ideal_session_valid_till');
        ...
        if (!$this->config->get('adyen_ideal_test')) {
            $this->data['action'] = 'https://live.adyen.com/hpp/details.shtml';
            $sharedSecret = $this->config->get('adyen_ideal_hmac_live');
        } else {
            $this->data['action'] = 'https://test.adyen.com/hpp/details.shtml';
            $sharedSecret = $this->config->get('adyen_ideal_hmac_test');
        }
        ...
        if ($order_info) {
            ...
            //Adyen Fields
            $this->data['merchantAccount'] = $this->config->get('adyen_ideal_email');
            $this->data['skinCode'] = $this->config->get('adyen_ideal_skincode');
            ...
            $this->data['business'] = $this->config->get('adyen_ideal_email');
            ...
            $this->data['return'] = $this->url->link('payment/adyen_ideal/callback');
...
//            $this->data['cancel_return'] = $this->url->link('payment/adyen/adyenCallback', '', 'SSL');
            ...
            $sign = $this->data['totalAmount'] . $this->data['currency_code'] . $this->data['shipBeforeDate'] . $this->data['merchantReference'] . $this->data['skinCode'] . $this->data['merchantAccount']
                    . $this->data['sessionValidity'] . $this->data['email'] . 'ideal' . $this->session->data['order_id'];
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ideal.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/payment/ideal.tpl';
            } else {
                $this->template = 'default/template/payment/ideal.tpl';
            }
    ...
    public function adyenCallback() {
        ...
        if ($order_info) {
            if (($this->request->post['eventCode'] == 'AUTHORISATION') && ($this->request->post['success'] == 'true')) {
                $order_status_id = $this->config->get('adyen_ideal_completed_status_id');
