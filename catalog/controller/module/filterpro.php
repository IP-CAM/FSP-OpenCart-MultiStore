...
class ControllerModuleFilterPro extends Controller {
    ...
    protected function index($setting) {
        if ($setting['type'] == 1) {
            ...
            if ($category_id) {
                ...
                $this->data['options'] = $this->model_module_filterpro->getOptions();
            }
            ...
            foreach ($this->data['options'] as $options) {
                if ($options['display'] == 'color' || $options['display'] == 'months') {
                    $option_ids[$options['display']] = $options['option_id'];
                }
            }
            $this->data['fullOptions'] = $this->model_module_filterpro->getFullOptions($option_ids);
    ...
    public function getProducts() {
        ...
        foreach ($products as $product) {
            ...
            $allOptions = $this->model_module_filterpro->getProductOptions($product['product_id']);
            $productOptions = '';
        ...
        $currencySymbol = $this->currency->getSymbolLeft($this->currency->getCode());
        $currencyValue = $this->currency->getValue($this->currency->getCode());
        ...
        //// get total product to show
        $totalproduct_to_show = array();
        ...
        $totalproduct_to_show_html = '<div class="limit showItem inline pull-left">' . $this->language->get('show_total') . '
            <select onchange="change_total(this.value)">';

        foreach ($totalproduct_to_show as $limits) {
            if ($limits['value'] == $limit) {
                $totalproduct_to_show_html .='<option value="' . $limits['value'] . '" selected="selected">' . $limits['text'] . '</option>';
            } else {
                $totalproduct_to_show_html .= '<option value="' . $limits['value'] . '">' . $limits['text'] . '</option>';
            }
        }

        $totalproduct_to_show_html .='</select></div>';

        $empty_products = '';
        if (empty($result)) {
            $empty_products = '<div class="warning margin-top-10 margin-left-20">Sorry, No Products !!</div>';
            // $empty_products = '<div class="alert alert-error"><h4>Sorry, No Products !!</h4></div>';
        }

        $json = json_encode(array('result' => $result, 'currencySymbol' => $currencySymbol, 'currencyValue' => $currencyValue, 'min_price' => $min_price,
            'empty_products' => $empty_products,'config_currency'=>$this->config->get('config_currency'),'curr_code'=>$this->currency->getCode(),
            'max_price' => $max_price, 'total_show' => $totalproduct_to_show_html, 'pagination' => $pagination->render() . $totalproduct_to_show_html,
