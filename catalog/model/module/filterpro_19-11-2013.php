...
class ModelModuleFilterPro extends Model {
    ...
    public function __construct($registry) {
        ...
        $filterProAllSettings = $this->getSettingFilterPro('filterpro', $this->config->get('config_store_id'));

        $this->filterpro_setting = $filterProAllSettings['filterpro_setting'];

        if (VERSION == '1.5.0') {
            $this->filterpro_setting = unserialize($this->config->get('filterpro_setting'));
        }
        $this->config_currency = 'EUR';
    }

    public function getAllChilds($category_id) {
        global $allCategoryIds;

        foreach ($category_id as $catid) {
            $catArr = array();
            $sqlNew = "SELECT category.category_id from category left join category_to_store on category_to_store.category_id=category.category_id where parent_id=" . $catid . " and category_to_store.store_id=" . (int) $this->config->get('config_store_id');

            $query = $this->db->query($sqlNew);

            foreach ($query->rows as $row) {
                $catArr[] = $row['category_id'];
                $allCategoryIds[] = $row['category_id'];
            }


            if (!empty($catArr)) {
                $this->getAllChilds($catArr);
            }
        }
    ...
    public function getAttributesByCategoryId($category_id) {
        ...
        $catArray = array('0' => $category_id);
        $allCategoryIds = array();

        $allCategoryIds = $this->getAllChilds($catArray);
        ...
        $sql = "SELECT DISTINCT pa.text, a.`attribute_id`, ad.`name`, ag.attribute_group_id, agd.name as attribute_group_name FROM `" . DB_PREFIX . "product_attribute` pa" .
                ...
                " WHERE  p.status = '1' AND p.date_available <= NOW() AND p2s.store_id ='" . (int) $this->config->get('config_store_id') . "' AND ag.store ='" . (int) $this->config->get('config_store_id') . "' ";
        /* if($category_id) {
          $sql .= " AND p2c.category_id = '" . (int)$category_id . "'";
          } */
        if (!empty($allCategoryIds)) {
            $sql .= " AND p2c.category_id IN('" . implode("','", $allCategoryIds) . "')";
        }

        $sql .= " AND pa.language_id = '1'" .
    ...
    public function getFullOptions($option_ids) {
        $options = array();

        if (is_array($option_ids) > 0) {
            if (!empty($option_ids['months'])) {
                $sql = "SELECT DISTINCT  ovd.name,ov.sort_order, od.name as 'option_name' 
	...
	WHERE ovd.language_id = '" . (int) $this->config->get('config_language_id') . "' 
            AND od.language_id = '" . (int) $this->config->get('config_language_id') . "'
             AND o.option_id = " . $option_ids['months'] . " order by ov.sort_order";

                $query = $this->db->query($sql);
                foreach ($query->rows as $row) {
                    $options['months'][$row['sort_order']] = $row['name'];
                }
            }
            if (!empty($option_ids['color'])) {
                $sql = "SELECT DISTINCT  ovd.name,ov.sort_order,  ov.image 
          FROM `" . DB_PREFIX . "option_value_description` ovd
    LEFT JOIN " . DB_PREFIX . "option_value ov ON(ovd.option_value_id=ov.option_value_id)
    LEFT JOIN " . DB_PREFIX . "option_description od ON(ov.option_id=od.option_id)
    LEFT JOIN `" . DB_PREFIX . "option` o ON(ov.option_id=o.option_id)
	WHERE ovd.language_id = '" . (int) $this->config->get('config_language_id') . "' 
            AND od.language_id = '" . (int) $this->config->get('config_language_id') . "'
             AND o.option_id = " . $option_ids['color'];

                $query = $this->db->query($sql);
                foreach ($query->rows as $row) {
                    $options['color'][$row['name']] = $row['image'];
                }
            }
        }
        return $options;
    ...
    public function getOptionsByCategoryId($category_id) {
        $sql = "SELECT DISTINCT ovd.option_value_id, ovd.*, od.name as 'option_name',ov.image,ov.html FROM `" . DB_PREFIX . "option_value_description` ovd
    LEFT JOIN " . DB_PREFIX . "option_value ov ON(ovd.option_value_id=ov.option_value_id)
    LEFT JOIN " . DB_PREFIX . "option_description od ON(ov.option_id=od.option_id)
    LEFT JOIN `" . DB_PREFIX . "option` o ON(ov.option_id=o.option_id)
	...
	WHERE category_id = '" . (int) $category_id . "' AND ovd.language_id = '" . (int) $this->config->get('config_language_id') . "' AND od.language_id = '" . (int) $this->config->get('config_language_id') . "'  AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id =" . (int) $this->config->get('config_store_id');
        ...
        foreach ($query->rows as $row) {
            ...
            $options[$row['option_id']]['option_values'][] = array(
    ...
    public function getQuantityLimits($data) {





        $sql2 = "select min(p.filter_min_qty) as min_price ,max(p.filter_max_qty) as max_price FROM " . DB_PREFIX . "product p" .
                " LEFT JOIN " . DB_PREFIX . "product_option_value pov ON (pov.product_id=p.product_id)" .
                " LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p2s.product_id=p.product_id)" .
                " LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2c.product_id=p.product_id)";

        $sql2 .= " WHERE p.status = '1' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int) $this->config->get('config_store_id');
        if ($data['category_id']) {
            $sql2 .= " AND p2c.category_id = '" . (int) $data['category_id'] . "'";
        }
    ...
    public function getTotalProducts($data) {
        ...
        $catArray = array('0' => $data['category_id']);
        $allCategoryIds = array();

        $allCategoryIds = $this->getAllChilds($catArray);
        $allCategoryIds[] = $data['category_id'];
...
//            $sql .= " AND p.filter_min_qty >=" . (int) ($data['min_quantity']);
//        }
//        if ($data['max_quantity'] > 0) {
//            $sql .= " AND p.filter_max_qty <=" . (int) ($data['max_quantity']);
//        }


        if ($data['min_price'] >= 0) {
            $min_price = $this->currency->convert($data['min_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND p.price >=" . round($min_price, 2);
        }
        if ($data['max_price'] > 0) {
            $max_price = $this->currency->convert($data['max_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND p.price <=" . round($max_price, 2);
        }
        ...
        if ($data['min_price'] >= 0) {
            $min_price = $this->currency->convert($data['min_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND realprice >=" . round($min_price, 2);
        }
        if ($data['max_price'] > 0) {
            $max_price = $this->currency->convert($data['max_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND realprice <=" . round($max_price, 2);
        }
    ...
    public function getTotalManufacturers($data) {
        ...
        if ($data['min_price'] >= 0) {
            $min_price = $this->currency->convert((int) $data['min_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND p.price >=" . round((int) $min_price,2);
        }
        ...
        if ($data['min_price'] >= 0) {
            $min_price = $this->currency->convert((int) $data['min_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND realprice >=" . round((int) $min_price,2);
        }
        if ($data['max_price'] > 0) {
            $max_price = $this->currency->convert($data['max_price'], $this->currency->getCode(), $this->config_currency);
            $sql .= " AND realprice <=" . round($max_price,2);
