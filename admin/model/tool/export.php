...
class ModelToolExport extends Model {
    ...
    function storeProductsIntoDatabase(&$database, &$products) {
        ...
        foreach ($products as $product) {
            ...
            $marketing_name = $database->escape($product['marketing_name']);
            ...
            $purchase_price = trim($product['purchase_price']);
            ...
            if ($product['language_id'] == 1) {

                $sql = "INSERT INTO `" . DB_PREFIX . "product` (`product_id`,`quantity`,`sku`,`upc`,`ean`,`jan`,`isbn`,`mpn`,`location`,";
                $sql .= "`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`purchase_price`,`points`,
                    `date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`season`,`status`,";
                ...
                $sql .= "$stockStatusId,'$model',$manufacturerId,'$imageName',$shipping,$price,$purchase_price,$points,";
                ...
                $sql .= "$weight,$weightClassId,'$season',$status,";
                $sql .= "$taxClassId,$viewed,$length,$width,$height,'$lengthClassId','$sort_order','$subtract','$minimum');";
                $sql2 = "INSERT INTO `" . DB_PREFIX . "product_description` (`product_id`,`language_id`,`name`,`description`,`meta_description`,`meta_keyword`,`meta_title`,`tag`,`mrkt_name`) VALUES ";
                $sql2 .= "($productId,$languageId,'$productName','$productDescription','$meta_description','$meta_keywords','$meta_title','$tags','$marketing_name');";
                $database->query($sql);
                $database->query($sql2);
                ...
                $chkpro = "select * from product where product_id=" . $productId;
                $chkproRes = $database->query($chkpro);
                if ($chkproRes->num_rows > 0) {
    ...
    function uploadProducts(&$reader, &$database) {
        ...
        for ($i = 0; $i < $k; $i+=1) {
            ...
            $marketing_name = $this->getCell($data, $i, $j++);
            $marketing_name = htmlentities($marketing_name, ENT_QUOTES, $this->detect_encoding($marketing_name));
            ...
            $purchase_price = $this->getCell($data, $i, $j++, '0.00');
            ...
            $season = $this->getCell($data, $i, $j++, 'true');
...
//            }
            ...
            $product['name'] = $name;
            $product['marketing_name'] = $marketing_name;
            ...
            $products[$productId . "-" . $langId] = $this->moreProductCells($i, $j, $data, $product);
        }
    ...
    function storeCategoriesIntoDatabase(&$database, &$categories) {
        ...
        foreach ($categories as $category) {
            ...
            $short_desc = $database->escape($category['short_desc']);
            ...
            if ($category['language_id'] == 1) {
                ...
                $sql2 = "INSERT INTO `" . DB_PREFIX . "category` (`category_id`, `image`, `parent_id`, `top`, `column`, `sort_order`, `date_added`, `date_modified`, `status`,`show_cat`) VALUES ";
                ...
                $sql2 .= " $status,$show_cat);";

                $database->query($sql2);


                $sql3 = "INSERT INTO `" . DB_PREFIX . "category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `meta_title`,`short_desc`) VALUES ";
                $sql3 .= "( $categoryId, $languageId, '$name', '$description', '$meta_description', '$meta_keywords','$meta_title','$short_desc' );";
                $database->query($sql3);
                if ($keyword) {
                ...
                $chkCat = "select * from category where category_id=" . $categoryId;
                $chkCatRes = $database->query($chkCat);
                if ($chkCatRes->num_rows > 0) {
                    $sql3 = "INSERT INTO `" . DB_PREFIX . "category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`,`meta_title`,`short_desc`) VALUES ";
                    $sql3 .= "( $categoryId, $languageId, '$name', '$description', '$meta_description', '$meta_keywords', '$meta_title','$short_desc' );";
                    $database->query($sql3);
                }
...
//            }
            ...
            $description = $this->getCell($data, $i, $j++);
            $description = htmlentities($description, ENT_QUOTES, $this->detect_encoding($description));
            $short_desc = $this->getCell($data, $i, $j++);
            $short_desc = htmlentities($short_desc, ENT_QUOTES, $this->detect_encoding($short_desc));
            $meta_description = $this->getCell($data, $i, $j++);
            $meta_description = htmlentities($meta_description, ENT_QUOTES, $this->detect_encoding($meta_description));
            $meta_keywords = $this->getCell($data, $i, $j++);
            $meta_keywords = htmlentities($meta_keywords, ENT_QUOTES, $this->detect_encoding($meta_keywords));
            ...
            $meta_title = htmlentities($meta_title, ENT_QUOTES, $this->detect_encoding($meta_title));
            $storeIds = $this->getCell($data, $i, $j++);
            $show_cat = $this->getCell($data, $i, $j++);
            
            $show_cat =strtoupper($show_cat)=="TRUE" ? 1 : 0;
            ...
            $categories[$categoryId . "-" . $langId] = $this->moreCategoryCells($i, $j, $data, $category);
        }
    ...
    function storeOptionsIntoDatabase(&$database, &$options) {
        ...
        foreach ($options as $option) {
            ...
            if (($type == 'select') || ($type == 'checkbox') || ($type == 'radio') || ($type == 'image') || ($type == 'information')) {
                if (!isset($optionValueIds[$name])) {
                    $optionValueIds[$name] = array();
                }
                if (!isset($optionValueIds[$name][$type])) {
...
//                    $sortOrder = ($option['sort_order'] == '') ? 0 : $option['sort_order'];
//                    $optionId = $optionIds[$name][$type];
//                    $sql = "UPDATE `" . DB_PREFIX . "option_value` SET  `sort_order`=$sortOrder ";
...
//            }
    ...
    function storeExtraProductsTabsIntoDatabase(&$database, &$ExtraProductsTabs) {
        $sql = "START TRANSACTION;\n";
        $sql .= "DELETE FROM `" . DB_PREFIX . "product_tab`;\n";
        $this->multiquery($database, $sql);

        // find the default language id
        $languageId = $this->getDefaultLanguageId($database);

        // Get Extra Products tabs from the database
        $sql = "SELECT * 
            FROM `" . DB_PREFIX . "tab_description` td
            LEFT JOIN `" . DB_PREFIX . "tab` t
            ON t.tab_id = td.tab_id";
        $result = $database->query($sql);
        $tabs = array();
        foreach ($result->rows as $row) {
            $name = $row['name'];
        ...
        // Store Data
        $first = TRUE;
        $sql = "INSERT INTO `" . DB_PREFIX . "product_tab` (`product_id`, `tab_id`, `language_id`, `text`, `position`, `show_empty`) VALUES ";
        foreach ($ExtraProductsTabs as $ExtraProductsTab) {
            $name = $ExtraProductsTab['name'];
            $product_id = $ExtraProductsTab['product_id'];
            $language_id = $ExtraProductsTab['language_id'];
            echo $text = $ExtraProductsTab['text'];
            if (isset($tabs[$name])) {
                $tab_id = $tabs[$name]['tab_id'];
                $position = $tabs[$name]['position'];
                $show_empty = $tabs[$name]['show_empty'];
                $sql .= ($first) ? "\n" : ",\n";
                $sql .= "($product_id, $tab_id, $language_id, '" . $text . "', $position, $show_empty)";
                $first = FALSE;
            }
        }
        if (!$first) {
    ...
    // hook function for reading additional cells in class extensions
    protected function moreExtraProductsTabsCells($i, &$j, &$worksheet, $reward) {
        return $reward;
    }

    function uploadExtraProductsTabs(&$reader, &$database) {
        $data = $reader->getSheet(8);
        $ExtraProductsTabs = array();
        $i = 0;
        $k = $data->getHighestRow();
        $isFirstRow = TRUE;
        for ($i = 0; $i < $k; $i+=1) {
            ...
            $language_id = $this->getCell($data, $i, $j++);
            $text = $this->getCell($data, $i, $j++);
            $ExtraProductsTabs[$i] = array();
            $ExtraProductsTabs[$i]['product_id'] = $productId;
            $ExtraProductsTabs[$i]['name'] = $customerGroup;
            $ExtraProductsTabs[$i]['language_id'] = $language_id;
            $ExtraProductsTabs[$i]['text'] = $text;
            $ExtraProductsTabs[$i] = $this->moreExtraProductsTabsCells($i, $j, $data, $ExtraProductsTabs[$i]);
        }
        return $this->storeExtraProductsTabsIntoDatabase($database, $ExtraProductsTabs);
    }
    ...
    protected function expectedCategoriesHeading() {
        return array("category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "language_id", "seo_keyword", "description","short_desc","meta_description", "meta_keywords", "meta_title", "store_ids","show_cat", "layout", "status\nenabled");
    }
    ...
    protected function expectedProductsHeading() {
        return array("product_id", "name", "marketing_name", "categories", "sku", "upc", "ean", "jan", "isbn", "mpn", "location", "quantity", "model", "manufacturer", "image_name", "requires\nshipping", "price", "purchase_price", "points", "date_added", "date_modified", "date_available", "weight", "unit", "length", "width", "height", "length\nunit", "season", "status\nenabled", "tax_class_id", "viewed", "language_id", "seo_keyword", "description", "meta_description", "meta_keywords", "meta_title", "stock_status_id", "store_ids", "layout", "related_ids", "tags", "sort_order", "subtract", "minimum");
    }
    ...
    function upload($filename) {
        ...
        try {
            ...
            $ok = $this->uploadExtraProductsTabs($reader, $database);
            if (!$ok) {
                return FALSE;
            }
            return $ok;
    ...
    protected function populateCategoriesWorksheet(&$worksheet, &$database, $languageId, &$boxFormat, &$textFormat) {
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('short_desc'), 32) + 1);
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('show_cat'), 16) + 1);
        ...
        $this->setCell($worksheet, $i, $j++, 'short_desc', $boxFormat);
        ...
        $this->setCell($worksheet, $i, $j++, 'show_cat', $boxFormat);
        ...
        foreach ($categories as $row) {
            ...
            $this->setCell($worksheet, $i, $j++, html_entity_decode($row['short_desc'], ENT_QUOTES, 'UTF-8'));
            ...
            $this->setCell($worksheet, $i, $j++, ($row['show_cat'] == 0) ? "false" : "true", $textFormat);
    ...
    protected function getProducts(&$database, $languageId) {
        ...
        $languageId1 = array(1,2,3,4);

        foreach ($languageId1 as $languageId) {
            $query = "SELECT ";
            $query .= "  p.product_id,";
            $query .= "  pd.name,";
            ...
            $query .= "WHERE pd.name!= 'NULL' ";
            $query .= "GROUP BY p.product_id ";
            $query .= "ORDER BY p.product_id, pc.category_id,pd.language_id; ";
            
            
            $result = $database->query($query);

            $resultantArry[$languageId] = $result->rows;
        }
       
        $grand = array_merge($resultantArry[1],$resultantArry[2],$resultantArry[3],$resultantArry[4]);
    ...
    function populateProductsWorksheet(&$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat) {
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('marketing_name'), 30) + 1);
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('purchase_price'), 10) + 1, $priceFormat);
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('season'), 5) + 1, $textFormat);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('status'), 5) + 1, $textFormat);
        ...
        $this->setCell($worksheet, $i, $j++, 'name', $boxFormat);
        $this->setCell($worksheet, $i, $j++, 'marketing_name', $boxFormat);
        ...
        $this->setCell($worksheet, $i, $j++, 'purchase_price', $boxFormat);
        ...
        $this->setCell($worksheet, $i, $j++, "season", $boxFormat);
        $this->setCell($worksheet, $i, $j++, "status\nenabled", $boxFormat);
        ...
        foreach ($products as $row) {
            ...
            $this->setCell($worksheet, $i, $j++, html_entity_decode($row['name'], ENT_QUOTES, 'UTF-8'));
             $this->setCell($worksheet, $i, $j++, html_entity_decode($row['mrkt_name'], ENT_QUOTES, 'UTF-8'));
            ...
            $this->setCell($worksheet, $i, $j++, $row['purchase_price'], $priceFormat);
            ...
            $this->setCell($worksheet, $i, $j++, ($row['season'] == 'spring') ? "spring" : "summer", $textFormat);
            $this->setCell($worksheet, $i, $j++, ($row['status'] == 0) ? "false" : "true", $textFormat);
    ...
    protected function getOptions(&$database, $languageId) {
        ...
        $query .= "  po.option_id,";
        $query .= "  ovd.name AS default_value,";
        $query .= "  po.required,";
    ...
    protected function getExtraProductsTabs(&$database, $languageId) {

        $query = "SELECT pt.product_id,td.language_id,pt.text,td.name 
                    FROM `" . DB_PREFIX . "product_tab` pt 
                    LEFT JOIN `" . DB_PREFIX . "tab_description` td
                    ON pt.tab_id = td.tab_id AND pt.`language_id` = td.`language_id`
                    ORDER BY pt.product_id, td.name";
        $result = $database->query($query);
        return $result->rows;
    }

    protected function populateExtraProductsTabsWorksheet(&$worksheet, &$database, $languageId, &$boxFormat, &$textFormat) {
        // Set the column widths
        $j = 0;
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('product_id') + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('name') + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('language_id') + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('text')), 32 + 1, $textFormat, TRUE);
        ...
        // The actual product Tabs data
        $i += 1;
        $j = 0;
        $rewards = $this->getExtraProductsTabs($database, $languageId);
        foreach ($rewards as $row) {
            $worksheet->getRowDimension($i)->setRowHeight(13);
            $this->setCell($worksheet, $i, $j++, $row['product_id']);
            $this->setCell($worksheet, $i, $j++, $row['name']);
            $this->setCell($worksheet, $i, $j++, $row['language_id']);
            $this->setCell($worksheet, $i, $j++, $row['text'], $textFormat, TRUE);
            $i += 1;
            $j = 0;
        }
    }
    ...
    function download() {
        ...
        try {
            ...
            $worksheet = $workbook->getActiveSheet();
            $worksheet->setTitle('Extra Products Tabs');
            $this->populateExtraProductsTabsWorksheet($worksheet, $database, $languageId, $boxFormat, $textFormat);
            $worksheet->freezePaneByColumnAndRow(1, 2);
