...
class ControllerSaleManageOrder extends Controller {
    ...
    public function exportpopup() {
        $this->load->model('sale/manageorder');
        $this->data['pendingOrderCount'] = $this->model_sale_manageorder->getOrderStatusCount(2);
        ...
        $url = '';
        $this->data['print_labels'] = $this->url->link('sale/manageorder/export', 'token=' . $this->session->data['token'] . $url, 'SSL');
...
//                    'vpadding' => 'auto',
                    ...
                    // EAN 8
                    $pdf->write1DBarcode($order_id, 'EAN8', $x + 10, $y + 45, 60, 18, 0.4, $style, 'N');
    ...
    public function printpicklist() {
        ...
        if (isset($this->request->post['selected']) && !empty($this->request->post['selected'])) {
            ...
            $pdf->SetFont('arialb', 'B', 8);
            $txt = "Order Id\n";
            $pdf->MultiCell(70, 50, $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT, $y + 15, true, 0, false, true, 0, 'T', false);
            $txt = "Customer\n";
            $pdf->MultiCell('', '', $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT + 20, $y + 15, true, 0, true, true, 0, 'T', false);
            $txt = "Products\n";
            $pdf->MultiCell('', '', $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT + 80, $y + 15, true, 0, true, true, 0, 'T', false);
            ...
            foreach ($this->request->post['selected'] as $key => $order_id) {
                ...
                $pdf->SetFont('arial', '', 8);
                $txt = $order_id . "\n";
                $pdf->MultiCell(70, 50, $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT, $y, true, 0, false, true, 0, 'T', false);
                ...
                $pdf->MultiCell('', '', $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT + 20, $y, true, 0, true, true, 0, 'T', false);
                ...
                foreach ($products as $product) {
                    ...
                    $pdf->SetFont('arial', '', 8);
                    $txt = $product['name'] . " ( " . $product['quantity'] . " ) " . "\n";
                    $pdf->MultiCell(70, 50, $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT + 80, $y + $product_y, true, 0, true, true, 0, 'T', false);
                    ...
                    if ($options) {
                        ...
                        foreach ($options as $option) {
                            ...
                            $pdf->MultiCell(70, 50, $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT + 85, $y + $product_y + 4, true, 0, true, true, 0, 'T', false);
                            $option_y += 4;
                            $product_y += $option_y;
                            $i++;
                        }
                ...
                if ($product_y + $option_y > 38) {
                    $line_height = $y + $product_y + $option_y - 5;
                } else {
                    $line_height = 38;
                }

//                $txt = $line_height."______________________________________________________________________________________________________________\n";
//                $pdf->MultiCell('', '', $txt, 0, 'J', false, 1, PDF_MARGIN_LEFT, $line_height, true, 0, true, true, 0, 'T', false);
                $i++;
            }
    ...
    public function updatestatus() {
        ...
        $formData['Password'] = 'account';
        if (isset($this->request->post['selected']) && !empty($this->request->post['selected'])) {
            foreach ($this->request->post['selected'] as $order_id) {
                $formData['keycode'] = $order_id;

                $RQ = new HTTP_Request('https://customer.abcmailgroup.com/cgi-bin/ParcelInfo.dll/track');
                $RQ->setMethod(HTTP_REQUEST_METHOD_POST);
                $RQ->addPostData('userid', 'Bloem01');
                $RQ->addPostData('Password', 'Tuin01');
                $RQ->addPostData('keycode', $order_id);
                $RQ->sendRequest();
                ...
                if ($vals) {
                    foreach ($vals as $val) {
                        if ($val['tag'] == 'PODSTATUS') {
                            $query = "SELECT * FROM `order_status` WHERE `name` ='" . $val['value'] . "'";
    ...
    public function export() {
        ...
        if (isset($this->request->post['selected']) && ($this->validateDelete())) {
            ...
            foreach ($result->rows as $row) {
                ...
                $total_val = preg_replace('/^[^0-9]+/', '', $result_total->row['text']);
            ...
            fwrite($fp, $response);
            fclose($fp);
            $fp = @fopen('abcmail.xml', 'r');

            // Add each line to an array
            if ($fp) {
                $array = explode("\n", fread($fp, filesize('abcmail.xml')));
            }
            if (isset($array) && !empty($array)) {
                ...
                if (isset($shipmentno)) {
                    ...
                    foreach ($orderID as $id) {
                        ...
                        $this->model_sale_manageorder->update($row['order_id'], $order_status_id);
//                        $update_status = "UPDATE  `" . DB_PREFIX . "order`  SET order_status_id=3 WHERE `order_id`=" . $id;
//                        $this->db->query($update_status);
                    }
                }
            }
    ...
    private function getList() {
        ...
        $this->data['processingOrderCount'] = $this->model_sale_manageorder->getOrderStatusCount(2);
